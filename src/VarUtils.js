    ...
    getVarName(completeVarName) {
        try {
            if (completeVarName.startsWith('_gen')) {
                let { groups: { variavel } } = /_gen(?<variavel>.+)\[\[(?<fem>.+)\]\].*\[\[(?<masc>.+)\]\].*/gm.exec(completeVarName)
                ...
                let { groups: { variavel, valor } } = /_sel(?<variavel>.+)_(?<valor>.+)_.+/gm.exec(completeVarName)
                variavel = variavel.trim()
                return variavel
            }

            else if (completeVarName.startsWith('IMAGE') && completeVarName.endsWith('()')) {
                const { groups: { variavel } } = /IMAGE\s+(?<variavel>.+)[(][)]/gm.exec(completeVarName)
                return variavel
            }
                ...
                let variavel = completeVarName.substring(7, completeVarName.length).trim()
                return variavel
            } else if (completeVarName.startsWith('@topic')) {
                let variavel = completeVarName.substring(6, completeVarName.length).trim()
                return variavel
            } else {
